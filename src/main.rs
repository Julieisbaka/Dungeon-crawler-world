...
impl App {
    async fn initialize_wgpu(&mut self, window: std::sync::Arc<Window>) {
        ...
        let config = wgpu::SurfaceConfiguration {
            ...
            width: size.width,
            height: size.height,
            present_mode: surface_caps.present_modes.iter().copied()
                .find(|mode| matches!(mode, wgpu::PresentMode::AutoVsync | wgpu::PresentMode::Fifo))
                .unwrap_or(surface_caps.present_modes[0]),
            alpha_mode: surface_caps.alpha_modes.iter().copied()
                .find(|mode| matches!(mode, wgpu::CompositeAlphaMode::Auto | wgpu::CompositeAlphaMode::PreMultiplied))
