...
impl UiPreviewManager {
    ...
    pub fn list_preview_names(&self) -> Vec<String> {
        Self::known_names()
            .iter()
    ...
    pub fn regenerate_grid_preview(&mut self) -> Result<(), String> {
        match self.windows.get_mut("grid") {
            Some(PreviewWindow::Grid { state, .. }) => {
                state.regenerate();
                Ok(())
            }
            Some(_) => Err("Grid preview is in an invalid state.".to_string()),
            None => Err("Grid preview is not open. Run 'invoke grid' first.".to_string()),
        }
    }

    pub fn reset_grid_preview_view(&mut self) -> Result<(), String> {
        match self.windows.get_mut("grid") {
            Some(PreviewWindow::Grid { state, .. }) => {
                state.reset_view();
                Ok(())
