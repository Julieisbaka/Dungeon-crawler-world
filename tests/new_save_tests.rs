...
fn test_real_time_generation() {
    ...
    assert_eq!(
        time, 432_000,
        "Real time should be exactly 432000 seconds (5 days)"
    );
}
...
fn test_normal_time_generation_range() {
    ...
    for _ in 0..100 {
        ...
        // 12h = 43200, 20h = 72000
        assert!(
            (43_200..=72_000).contains(&time),
            "Normal time should be between 12h and 20h in seconds, got {}",
            time
        );
    }
}
...
fn test_stat_generation_ranges() {
    ...
    for _ in 0..100 {
        ...
        assert!(
            (3..=5).contains(&walking),
            "Walking out of range: {}",
            walking
        );
        assert!(
            (3..=5).contains(&swimming),
            "Swimming out of range: {}",
            swimming
        );
        assert!(
            (3..=5).contains(&breathing),
            "Breathing out of range: {}",
            breathing
        );
        assert!(
            (1..=8).contains(&strength),
            "Strength out of range: {}",
            strength
        );
        assert!(
            (3..=5).contains(&intelligence),
            "Intelligence out of range: {}",
            intelligence
        );
        assert!(
            (2..=6).contains(&dexterity),
            "Dexterity out of range: {}",
            dexterity
        );
        assert!(
            (2..=4).contains(&charisma),
            "Charisma out of range: {}",
            charisma
        );
        assert!(
            (2..=6).contains(&constitution),
            "Constitution out of range: {}",
            constitution
        );
    }
}
