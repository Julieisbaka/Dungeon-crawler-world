...
fn test_cell_has_four_neighborhoods() {
    ...
    for row in &grid.cells {
        for cell in row {
            assert_eq!(
                cell.neighborhoods.len(),
                4,
                "Each cell should have 4 neighborhoods"
            );
        }
    }
}
...
fn test_neighborhood_has_nodes() {
    ...
    for neighborhood in &cell.neighborhoods {
        assert!(
            !neighborhood.nodes.is_empty(),
            "Neighborhood should have nodes"
        );
        assert!(
            neighborhood.nodes.len() >= 5 && neighborhood.nodes.len() <= 15,
    ...
    for (y, row) in grid.cells.iter().enumerate() {
        for (x, cell) in row.iter().enumerate() {
            ...
            for neighborhood in &cell.neighborhoods {
                for node in &neighborhood.nodes {
                    assert!(
                        node.x >= cell_min_x && node.x <= cell_max_x,
                    ...
                    assert!(
                        node.y >= cell_min_y && node.y <= cell_max_y,
    ...
    for row in &grid.cells {
        for cell in row {
            for neighborhood in &cell.neighborhoods {
                ...
                let bathrooms: Vec<(f32, f32)> = neighborhood
                    .nodes
                    .iter()
